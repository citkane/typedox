...
export default function () {
	...
	it('errors on malformed kind', function () {
		...
		wrapper.wrap(checker, program, {} as ts.Node);
	...
	it('gets node and type from a symbol', function () {
		let nodesAndType!: ReturnType<typeof utils.getNodesAndTypeFromSymbol>;
		const symbol = sourceType.getProperty('localFunc')!;
		assert.exists(symbol);
		assert.doesNotThrow(
			() =>
				(nodesAndType = utils.getNodesAndTypeFromSymbol(
					checker,
					symbol,
				)),
		);
		...
		const noDeclarations = doxStub.deepClone(symbol);
	...
	it('wraps a node', function () {
		...
		assert.exists(node);
		assert.doesNotThrow(() => wrapper.wrap(checker, program, node));
		const wrap = wrapper.wrap(checker, program, node); // should hit the cache to complete coverage
		assert.exists(wrap);
		assert.equal(
			wrap!.nodeText,
	...
	it('retrieves local export declaration', function () {
		...
		const aliasWrap = wrapper.wrap(checker, program, alias!);
	...
	it('gets a targetFileName from a namespaceExport', function () {
		...
		const wrap = wrapper.wrap(checker, program, specifier!);
		assert.exists(wrap);
	...
	it('gets a targetFileName from a namespaceImport', function () {
		...
		const importerWrap = wrapper.wrap(checker, program, specifier!);
		...
		const wrap = wrapper.wrap(checker, program, importer!);
		assert.exists(wrap);
		assert.equal(confChild, wrap!.targetFileName);
	});
	...
	it('gets a targetFileName from a exportSpecifier', function () {
		...
		const wrap = wrapper.wrap(checker, program, exporter);
		assert.exists(wrap);
		assert.equal(confChild, wrap!.targetFileName);
	});
	it('gets a targetFileName from a ExportDeclaration', function () {
		...
		const wrap = wrapper.wrap(checker, program, declaration!);
		assert.exists(wrap);
		assert.equal(confChild, wrap!.targetFileName);
	});
	...
	it('wraps a loner type export', function () {
		...
		const typeWrap = wrapper.wrap(checker, program, isTypeOnly!);
	...
	it('wraps a namespace export', function () {
		...
		assert.exists(child);
		const exportWrap = wrapper.wrap(checker, program, child!);
	...
	it('wraps a local export', function () {
		...
		const exWrap = wrapper.wrap(checker, program, ex!);
	...
	it('wraps a remote export', function () {
		...
		const remoteExportWrap = wrapper.wrap(checker, program, remoteExport!);
	...
	it('wraps a function', function () {
		...
		const fncWrap = wrapper.wrap(checker, program, fnc!);
	...
	it('wraps a class', function () {
		...
		assert.doesNotThrow(() => wrapper.wrap(checker, program, IsClass!));
	});
	it('wraps a exportStar', function () {
		const confChild = path.join(projectDir, 'child/child.ts');
		assert.isTrue(wrapper.isExportStar(starExport));

		const starWrap = wrapper.wrap(checker, program, starExport!);
	...
	it('wraps items in the factory as symbol and node', function () {
		((sourceFile as any).locals as Map<string, ts.Symbol>).forEach(
			(local) => {
				...
				const wrap = wrapper.wrap(checker, program, local);
				assert.equal(error, '', error);
				assert.exists(wrap);
				wrap!.cacheFlush();
				error = '';
		...
		sourceSymbol.exports?.forEach((exported) => {
			...
			const wrap = wrapper.wrap(checker, program, exported);
			assert.equal(error, '', error);
			assert.exists(wrap);
			wrap!.cacheFlush();
			error = '';
		...
		sourceFile?.forEachChild((node) => {
			if (!wrapper.isSpecifierKind(node.kind)) return;
			let error = '';
			errorStub = stub(log, 'error').callsFake((...args) => {
				console.log(args[1]);
				error = args[1];
			...
			const wrap = wrapper.wrap(checker, program, node);
			assert.equal(error, '', error);
			assert.exists(wrap);
			wrap!.cacheFlush();
			error = '';
...
function hasGeneratedCommonItems(
	...
	const { tsNode, tsSymbol, tsType } = wrapped;

	assert.isTrue(
		tsNode && (ts.isSourceFile(tsNode) || wrapper.isNode(tsNode)),
	);
	assert.isTrue(tsSymbol && wrapper.isSymbol(tsSymbol));
	assert.isTrue(tsType && wrapper.isTypeNode(tsType));
	assert.equal(wrapped.name, name);
	assert.equal(
		kind,
		wrapped.kind,
		`kinds did not match, got ${
			wrapped.kind && ts.SyntaxKind[wrapped.kind]
		}`,
	);
	assert.equal(nodeFlag, wrapped.nodeFlag, ts.NodeFlags[wrapped.nodeFlag]);
	assert.equal(
		symbolFlags,
		wrapped.symbolFlag,
		`symbolFlags did not match, got ${ts.SymbolFlags[wrapped.symbolFlag]}`,
	);
	assert.equal(
		typeFlags,
		wrapped.typeFlag,
		`typeFlags did not match, got ${ts.TypeFlags[wrapped.typeFlag]}`,
	);

	assert.equal(ts.SyntaxKind[kind], wrapped.kindString);
	assert.equal(ts.NodeFlags[nodeFlag], wrapped.nodeFlagString);
	assert.equal(ts.SymbolFlags[symbolFlags], wrapped.symbolFlagString);
	assert.equal(ts.TypeFlags[typeFlags], wrapped.typeFlagString);
	assert.equal(wrapped.fileName, sourceFile!.fileName);
}
