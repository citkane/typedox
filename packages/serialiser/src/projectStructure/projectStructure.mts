...
import { serialisedBranch } from '../index.mjs';
...
export function serialiseDoxProject(project: DoxProject) {
...
export function serialiseDoxReference(reference: DoxReference) {
	if (!reference.doxBranch) return notices.reference.noBranch(reference);
	//const serialisedReference = {} as Record<string, serialisedBranch>;
	//serialisedReference[reference.name] = serialiseBranch(reference.doxBranch!);
	return serialiseBranch(reference.doxBranch!);
}
...
const notices = {
	reference: {
		noBranch: function (reference: DoxReference) {
			log.error(
				log.identifier(__filename),
				'Did not find a doxBranch on a doxReference',
				reference.name,
			);
		},
	},
