 ...
 */
export class Relation extends Logger {
	public get: dox.TscWrapper;
	private name: string;
	private filesMap: dox.fileMap;
	private declaration: dox.TsDeclaration;
	private context: dox.lib.DoxContext;
	private tsWrap: dox.TsDeclaration['tsWrap'];

	constructor(context: dox.lib.DoxContext, declaration: dox.TsDeclaration) {
		super();
		Relation.classString.bind(this);

		this.declaration = declaration;
		this.get = declaration.get;
		this.context = context;
		this.tsWrap = declaration.tsWrap;
	...
	private mapExportSpecifier(get: dox.TscWrapper) {
		if (get.localTargetDeclaration) {
			...
			const localGet = this.declaration.tsWrap(local);
	...
	private registerReExporter(symbol: ts.Symbol) {
		dox.parseExportStars.call(this, symbol).forEach((expression) => {
