...
import { tsc, log, logLevels, TsWrapper, config, Dox } from 'typedox';
...
export default function () {
	...
	it('wraps a valid kind', function () {
		sourceFile?.forEachChild((node) => {
			...
			let wrap!: TsWrapper;
			assert.doesNotThrow(
				() => (wrap = tsc.wrap(checker, program, node)),
			);
			wrap.cacheFlush();
		});
	});
	...
	it('wraps a reExportChild', function () {
		...
		const exportChild = (starExport.declarations![0] as any)[
			'moduleSpecifier'
		];
