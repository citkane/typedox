...
it('maps a export= ExportAssignment', function () {
	const result = this.testSpecifier('export=', 'exportEqual/exportEqual.ts');
	const { children, declaration } = result;
...
it('maps a default array Common export', function () {
	const result = this.testSpecifier('export=', 'default/array.ts', 'common');
	const { declaration, children } = result;
	assert.sameMembers(children, []);
...
it('maps a default class Common export', function () {
	const result = this.testSpecifier('export=', 'default/class.ts', 'common');
	const { declaration, children } = result;
	assert.sameMembers(children, []);
...
it('maps a default enum Common export', function () {
	const result = this.testSpecifier('export=', 'default/enum.ts', 'common');
	const { declaration, children } = result;
	assert.sameMembers(children, []);
...
it('maps a default object Common export', function () {
	const result = this.testSpecifier('export=', 'default/object.ts', 'common');
	const { declaration, children } = result;
	assert.sameMembers(children, []);
	testDefault(
		declaration,
...
function testSpecifier(
	...
	factory: factoryFolders = localFactory,
) {
